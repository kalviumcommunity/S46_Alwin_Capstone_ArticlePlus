...
import TagRibbion from "./TagRibbion"
import { Link, useSearchParams } from "react-router-dom"
import { useEffect, useRef, useState } from "react"

const ArticleCard = ({ article }) => {
    return (
        <div className="group flex h-fit flex-col gap-3 px-3 py-2 hover:cursor-pointer">
            <img
                className="h-46 aspect-[4/3] rounded-sm object-cover"
                src={article.image}
                alt=""
            />
            <div className="flex flex-col justify-center gap-2">
                <span className="font-serif text-lg font-medium leading-6 group-hover:underline group-hover:underline-offset-4">
                    {article.title}
                </span>
        ...
        <>
            ...
            <div className="wrapper flex-col pb-10">
                <div className="flex flex-col gap-2 py-6">
                    <span className="ml-3 font-serif text-xl font-normal">
                        Latest
                    </span>
                    <div className="grid grid-cols-4 divide-x first:pl-0 last:pr-0">
                ...
                <div className="flex flex-col gap-2 py-6">
                    <div className="grid grid-cols-1 divide-x">
                        {highlight.map((article) => (
                            <div className="group flex h-fit gap-6 bg-gray-50 px-6 py-10 hover:cursor-pointer">
                                <img
                                    className="aspect-[4/3] w-1/2 rounded-sm object-cover"
                                    src={article.image}
                                    alt=""
                                />
                                <div className="flex flex-col justify-center gap-2 px-12 text-center">
                                    <span className="font-serif text-3xl font-medium group-hover:underline group-hover:underline-offset-4">
                                        {article.title}
                                    </span>
                                    <span className="line-clamp-4 text-base font-normal text-gray-500">
                                        {article.subtitle}
                                    </span>
                                    <span className="text-sm font-semibold leading-4">
                                        {article.author.name}
